...
export default function Sidebar({ current, handleOpenHiddenBar, unit, setUnit, isHiddenBarOpen, handleCloseHiddenBar, searchTerm, handleSearchChange, handleSearchSubmit }) {
  const iconSrc = `/img/${current.icon || 'Default'}.png`;

  return (
    <div className="bg-[#1e213a] text-center rounded-lg lg:rounded-l-lg p-6 lg:p-10 flex flex-col lg:w-1/3 mb-0 lg:mb-0">
      <div className="flex items-center justify-between mb-5">
        <div className="w-[161px] h-[40px] bg-[#6e707a] text-[#e7e7eb] px-4 py-2 shadow-md cursor-pointer hover:bg-[#444242] transition duration-400 ease-in-out flex items-center justify-center sm:ml-4 lg:ml-0" onClick={handleOpenHiddenBar}>
          Search for places
        </div>
        <div className="text-[#e7e7eb] text-2xl h-9 w-9 flex items-center justify-center rounded-full bg-white bg-opacity-20 shadow-md cursor-pointer hover:bg-[#444242] transition duration-400 ease-in-out ml-4" onClick={handleOpenHiddenBar}>
          <LocationSearching />
        </div>
      </div>
